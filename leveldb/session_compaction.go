...
func (s *session) flushMemdb(rec *sessionRecord, mdb *memdb.DB, maxLevel int) (int, error) {
	// Create sorted table.
	iter := mdb.NewIterator(nil)  // 根据memdb创建一个迭代器
	defer iter.Release()
	t, n, err := s.tops.createFrom(iter)  // 创建一个sstable文件，把内容填到sstable
	if err != nil {
		return 0, err
	}
...
func (s *session) pickCompaction() *compaction {
	...
	// v的合并场景，采用轮转的方法选择起始输入文件，
	// 记录了上一次该层合并的文件的最大key，下一次则选择在此key之后的首个文件。
	if v.cScore >= 1 {
		// 将得分最高的层数记录，若该得分超过1，则为下一次进行合并的层数。
		sourceLevel = v.cLevel
		cptr := s.getCompPtr(sourceLevel)  // 获取需要压缩的key
		tables := v.levels[sourceLevel]
		for _, t := range tables {
			if cptr == nil || s.icmp.Compare(t.imax, cptr) > 0 {
				t0 = append(t0, t)  // 找出与cptr重叠的部分(t.imax > cptr)
				break
			}
		}
		if len(t0) == 0 {
...
func (s *session) getCompactionRange(sourceLevel int, umin, umax []byte, noLimit bool) *compaction {
	...
	// 获取v.levels[sourceLevel]与区间[umin, umax]重叠的部分
...
func (c *compaction) newIterator() iterator.Iterator {
	...
	for i, tables := range c.levels {
		...
		// 如果需要压缩的层次是level0，由于sst之间会相互重叠，因此需要吧所有的迭代器加入进去。
		if c.sourceLevel+i == 0 {
			for _, t := range tables {
				its = append(its, c.s.tops.newIterator(t, nil, ro))
			}
		} else {
			// 非level-0，一层只需要添加一个iterator即可。
			// 后面的level都是有序的，一个level维护一个iterator即可。
			it := iterator.NewIndexedIterator(tables.newIndexIterator(c.s.tops, c.s.icmp, nil, ro), strict)
			its = append(its, it)
		}
	}

	return iterator.NewMergedIterator(its, c.s.icmp, strict)  // 合并成一个大的iterator
}

...
func (db *DB) recoverJournal() error {
	...
	rawFds, err := db.s.stor.List(storage.TypeJournal)  // 获取所有的日志目录fd
	if err != nil {
		return err
	}
	sortFds(rawFds)
	...
	var (
		ofd storage.FileDesc // Obsolete file.过时的文件集合ofd
		rec = &sessionRecord{}
	)
	...
	if len(fds) > 0 {
		...
		for _, fd := range fds {
			...
			for {
				...
				if mdb.Size() >= writeBuffer {  // 写够4mb，进行落盘操作
					if _, err := db.s.flushMemdb(rec, mdb, 0); err != nil {
						fr.Close()
						return err
					}
	...
	if !ofd.Zero() {
		db.s.stor.Remove(ofd)  // 把所有的过时文件删除
	}
...
// 与recoverJournalR0的区别在于该函数不会删除任何过时的日志文件。
// 还有，仅仅只会进行构建memdb的步骤，但不会进行flushMemdb的操作
...
func memGet(mdb *memdb.DB, ikey internalKey, icmp *iComparer) (ok bool, mv []byte, err error) {
	// 同一个key，不同的seqno，对应的也是不同的internalKey
	mk, mv, err := mdb.Find(ikey)  // 从跳表中查找
	if err == nil {
		// 反解这个内部key
		...
		if icmp.uCompare(ukey, ikey.ukey()) == 0 {
			if kt == keyTypeDel {  // 如果kt是删除，返回不存在
...
func (db *DB) get(auxm *memdb.DB, auxt tFiles, key []byte, seq uint64, ro *opt.ReadOptions) (value []byte, err error) {
	...
	for _, m := range [...]*memDB{em, fm} {
		...
		// 从memdb/frozen memdb中获取
		if ok, mv, me := memGet(m.DB, ikey, db.s.icmp); ok {
			return append([]byte{}, mv...), me
		}
	}

	// memdb和frozen-memdb都找不到
	v := db.s.version()  // 获取version
	value, cSched, err := v.get(auxt, ikey, ro, false)
	v.release()
	if cSched {
		// Trigger table compaction.
		db.compTrigger(db.tcompCmdC)  // cSeek不为nil，触发压缩
	}
	return
}

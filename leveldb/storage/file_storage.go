...
func (lock *fileStorageLock) Unlock() {
	if lock.fs != nil {
		...
		if lock.fs.slock == lock {
			lock.fs.slock = nil  // 只有对应的fileStorageLock才能解锁
		}
	}
}
...
func (fs *fileStorage) doLog(t time.Time, str string) {
	if fs.logSize > logSizeThreshold {  // 超过1mb大小进行日志文件的回滚
		// Rotate log file.
		fs.logw.Close()
		fs.logw = nil
		fs.logSize = 0
		rename(filepath.Join(fs.path, "LOG"), filepath.Join(fs.path, "LOG.old"))  // 备份旧的日志文件
	}
	if fs.logw == nil {
		var err error
...
func (fs *fileStorage) setMeta(fd FileDesc) error {
	...
	if _, err := os.Stat(currentPath); err == nil {
		...
		// writeFileSynced：不带缓冲写，写完直接落盘
		// 将CURRENT的内容写入到bak文件中
	...
	// 每个文件的元数据内容均写入到fs.path/CURRENT.{fd.Num}
	...
	// Sync root directory.
	// 这里为什么要这么做?
	// 干货: fsync 把文件描述符fd指向的文件缓存在内核中的所有已修改的数据写入文件系统，
	// 包含数据与文件元数据（文件大小，文件修改时间等）。
	// 但是fsync不会写入对指向文件的目录项的修改，也就是说如果新创建了一个文件，要是确保下次能正确读出的话，就需要把所在目录也fsync一下。

	// 上面涉及了新创建文件操作，因此需要对目录进行fsync操作
...
func (fs *fileStorage) List(ft FileType) (fds []FileDesc, err error) {
	...
	names, err := dir.Readdirnames(0)  // 返回目录里面的所有文件名
	// Close the dir first before checking for Readdirnames error.
...
func (fs *fileStorage) Close() error {
	...
	// Clear the finalizer.
	// 去掉Finalizer注册，因为已经进入该函数了，否则后续GC回收fs会重复调用Close
...
type fileWrap struct {
	*os.File
	fs     *fileStorage  // 维护文件存储系统的信息，和打log
	fd     FileDesc
	closed bool
}

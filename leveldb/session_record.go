...
func (p *sessionRecord) resetAddedTables() {
	p.hasRec &= ^(1 << recAddTable)  // 清空该位
	p.addedTables = p.addedTables[:0]
}

// 删除一个表不会对原来的addTables进行修改，而是往deleteTables里面添加元素

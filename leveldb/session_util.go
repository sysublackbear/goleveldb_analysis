...
// 由newSession里面开了goroutine进行调用: go s.refLoop()
...
func (s *session) version() *version {
	s.vmu.Lock()
	defer s.vmu.Unlock()
	s.stVersion.incref()  // session对应的stVersion引用计数加一
	return s.stVersion
}
...
func (s *session) newManifest(rec *sessionRecord, v *version) (err error) {
	// allocFileNum(): atomic.AddInt64(&s.stNextFileNum, 1) - 1
	// 新建fd
	fd := storage.FileDesc{Type: storage.TypeManifest, Num: s.allocFileNum()}
	// 新建storage.Writer(类io.Writer)
	...
	// 给rec赋值
	s.fillRecord(rec, true)  // 创建snapshot session record?
	// 针对每一层，给rec赋值
	v.fillRecord(rec)

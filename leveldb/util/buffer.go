...
func (b *Buffer) grow(n int) int {
	...
	if len(b.buf)+n > cap(b.buf) {
		...
		if b.buf == nil && n <= len(b.bootstrap) {
			buf = b.bootstrap[0:]  // 小内存,直接从b.bootstrap中获取内存
...
func (b *Buffer) readSlice(delim byte) (line []byte, err error) {
	i := bytes.IndexByte(b.buf[b.off:], delim)  // 找出delim在b.buf[b.off:]第一次出现的位置下标
	end := b.off + i + 1
	if i < 0 {
		end = len(b.buf)
		err = io.EOF

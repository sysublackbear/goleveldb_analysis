...
func (b *block) seek(cmp comparer.Comparer, rstart, rlimit int, key []byte) (index, offset int, err error) {
	// 在[rstart, rlimit]区间中二分找出满足匿名函数条件的下标
	index = sort.Search(b.restartsLen-rstart-(b.restartsLen-rlimit), func(i int) bool {
		// todo: 这句话看不懂
		offset := int(binary.LittleEndian.Uint32(b.data[b.restartsOffset+4*(rstart+i):]))
		offset++                                    // shared always zero, since this is a restart point
		v1, n1 := binary.Uvarint(b.data[offset:])   // key length,读出key的长度
		_, n2 := binary.Uvarint(b.data[offset+n1:]) // value length,读出value的长度
		m := offset + n1 + n2
		return cmp.Compare(b.data[m:m+int(v1)], key) > 0
	}) + rstart - 1  // todo: 为什么要加上rstart - 1
	if index < rstart {
...
// 从指定的offer，在block中读取出对应的entry结构(key, value, nShared, n, err)
func (b *block) entry(offset int) (key, value []byte, nShared, n int, err error) {
	...
	// 从b.data中解码uint64并返回该值和读取的字节数
	// 分别读取出v0,v1,v2
	v0, n0 := binary.Uvarint(b.data[offset:])       // Shared prefix length（前缀）
	v1, n1 := binary.Uvarint(b.data[offset+n0:])    // Key length（key的长度）
	v2, n2 := binary.Uvarint(b.data[offset+n0+n1:]) // Value length（value的长度）
	// m:总共占的字节个数
	m := n0 + n1 + n2
	n = m + int(v1) + int(v2)
	...
	key = b.data[offset+m : offset+m+int(v1)]  // 读取key的值
	value = b.data[offset+m+int(v1) : offset+n]  // 读取value的值
	nShared = int(v0)
	return
}

func (b *block) Release() {
	b.bpool.Put(b.data)  // 回收该块内存空间
	b.bpool = nil
	b.data = nil
}
...
func (i *blockIter) reset() {
	if i.dir == dirBackward {
		i.prevNode = i.prevNode[:0]  // 为什么不直接i.prevNode = []？
...
func (i *indexIter) Get() iterator.Iterator {
	value := i.Value()  // indexIter匿名嵌入了blockIter
	if value == nil {
		return nil
	}
	dataBH, n := decodeBlockHandle(value)  // 从value中取出offset和length
	if n == 0 {
...
// 分4种block:meta-block,index-block,filter-block,data-block
...
func (r *Reader) readRawBlock(bh blockHandle, verifyChecksum bool) ([]byte, error) {
	// 从内存池申请内存
	data := r.bpool.Get(int(bh.length + blockTrailerLen))  // 从内存池中取出bh.length+blockTrailerLen长度大小的buffer
	// 在指定的偏移:bh.offset读取data长度的数据
	// 从sstable读取数据写到data（内存池buffer中）
	...
	case blockTypeSnappyCompression:
		...
		data = decData
	default:  // data[bh.length](blockType不合法)
		r.bpool.Put(data)  // 回收内存
...
func (r *Reader) readBlock(bh blockHandle, verifyChecksum bool) (*block, error) {
	...
	restartsLen := int(binary.LittleEndian.Uint32(data[len(data)-4:]))  // 最后4个字节，restart点的个数
	// 组装block结构
	b := &block{
		bpool:          r.bpool,
		bh:             bh,
		data:           data,  // 真正的数据
		restartsLen:    restartsLen,  // restart点的个数
		restartsOffset: len(data) - (restartsLen+1)*4,  // 真正数据段的末尾,trailer的开头
	}
	return b, nil
}
	...
	if r.cache != nil {
		...
		if fillCache {
			...
			ch = r.cache.Get(bh.offset, func() (size int, value cache.Value) {
				...
				return cap(b.data), b  // 先readBlock读取出一个block，然后把block的cap(b.data), b写入到cache中
			})
		} else {
...
func (r *Reader) getIndexBlock(fillCache bool) (b *block, rel util.Releaser, err error) {
	if r.indexBlock == nil {
		// indexBlock为空，尝试从cached获取，cache获取不到，由sstable读取写入到cache中
...
func (r *Reader) newBlockIter(b *block, bReleaser util.Releaser, slice *util.Range, inclLimit bool) *blockIter {
	bi := &blockIter{
		tr:            r,  // 把Reader放进去,目前不清楚tr有什么用
		block:         b,  // block
		blockReleaser: bReleaser,  // blockReleaser
	...
	if slice != nil {
		// util.Range{Start, Limit}
		if slice.Start != nil {
			if bi.Seek(slice.Start) {  // slice.Start是个key
...
func (r *Reader) find(key []byte, filtered bool, ro *opt.ReadOptions, noValue bool) (rkey, value []byte, err error) {
	...
	// index block找到这个key了，定位到具体的data block中查找
...
func NewReader(f io.ReaderAt, size int64, fd storage.FileDesc, cache *cache.NamespaceGetter, bpool *util.BufferPool, o *opt.Options) (*Reader, error) {
	...
	r := &Reader{
		fd:             fd,  // 描述(入参确定)
		reader:         f,   // 文件操作对象io.ReaderAt(入参确定)
		cache:          cache,  // cache(入参确定)
		bpool:          bpool,  // 内存池
		o:              o,      // 配置
		cmp:            o.GetComparer(),  // 配置函数
		verifyChecksum: o.GetStrict(opt.StrictBlockChecksum),   // 是否严格检查校验和
	}

	if size < footerLen {  // size太小,footer固定大小都为48字节
		r.err = r.newErrCorrupted(0, size, "table", "too small")
		return r, nil
	}

	footerPos := size - footerLen  // footer的下标
	...
	// Read metaindex.
	// 在metaBlock的基础上建立BlockIter
	// 遍历meta index block，查看是否存在"有用"的filter block的索引信息；
